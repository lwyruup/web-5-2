...
if (empty($_POST['name'])) {
  ...
  if (!empty($_COOKIE[session_name()]) &&
    ...
    try {
      ...
      $stmt = $db->prepare("UPDATE application5 SET name = ?, email = ?, year = ?, gender = ?, bio = ? WHERE id =?");
      $stmt -> execute(array(
          $_POST['name'],
          $_POST['email'],
          $_POST['year'],
      ...
      $sth = $db->prepare("DELETE FROM Languages5 WHERE id = ?");
      $sth->execute(array($user_id));
    ...
    try {
      $stmt = $db->prepare("INSERT INTO application5 SET name = ?, email = ?, year = ?, gender = ?, bio = ?");//, login = ?, password = ?
      $stmt -> execute(array(
          $_POST['name'],
          $_POST['email'],
          $_POST['year'],
